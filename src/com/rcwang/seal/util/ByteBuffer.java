...
public final class ByteBuffer {
  ...
  public void remove(int begin, int length) {
    System.arraycopy(_buffer, begin + length, _buffer, begin, _capacity
        - length - begin);
  ...
  public short getShort(int i) {
    ...
    return (short) (((_buffer[i] & 0xff) << 8) + (_buffer[i + 1] & 0xff));
  }
  ...
  public int getInt(int i) {
    ...
    return (((_buffer[i + 0] & 0xff) << 24) + ((_buffer[i + 1] & 0xff) << 16)
        + ((_buffer[i + 2] & 0xff) << 8) + ((_buffer[i + 3] & 0xff)));
  }
  ...
  static class BBInputStream extends InputStream {
    ...
    public int read() throws IOException {
      if (_index >= _buf._length)
        return -1;
      else return _buf._buffer[_index++] & 0xff;
    }
