...
import com.rcwang.seal.fetch.OfflineSearcher;
...
public class OfflineSeal extends Pinniped {
    ...
    public static Logger log = Logger.getLogger(OfflineSeal.class);
    ...
    public static void main(String args[])  {
        ...
        if (args.length == 0) {
            ...
            log.info("Usage 1: java " + OfflineSeal.class.getName() + " seed_file [hint_file]");
            log.info("Usage 2: java " + OfflineSeal.class.getName() + " seed_1 seed_2 ...");
            return;
        }
...
//        seal.saveResults();
//        seal.saveWrappers();
        seal.save(); // rolled results and wrappers into SetExpander, kmr 5/11/2012
        
        System.out.println(seal.getEntityList().toDetails(100, seal.getFeature()));
    ...
    public static DocumentSet fetch(EntityList seeds, String hint, String langID, 
        ...
        OfflineSearcher offlineSearcher = new OfflineSearcher(gv.getIndexDir(),gv.getLocalRoot());
        offlineSearcher.setLangID(langID);
        offlineSearcher.setNumResults(gv.getNumResults());
        offlineSearcher.setUseEngine(engine);
        // this ignores relational separators, since it doesn't call
        // the Entity.splitRelation method....
        // DocumentSet documents = offlineSearcher.fetchDocuments(seeds.getEntityNames(), hint);
        ArrayList<String> seedStrings = new ArrayList<String>();
        // add queries into the searcher
        for (String[] seed : seeds.getEntityNames()) {
            for (String s : seed)
                seedStrings.add(s);
        }
        DocumentSet documents = offlineSearcher.fetchDocuments(seedStrings, hint);
        int numUrlsFetched = documents.size();
    
        // output the percentage of URLs fetched
        log.info("Number of webpages retrieved from cache: " + numUrlsFetched);
        return documents;
    }
    ...
    public OfflineSeal(String langID) {
        this();
        setLangID(langID);
    }
    ...
    private Set<EntityLiteral> extract(EntityList seeds, DocumentSet documents) {
        ...
        for (int i = 0; i < documents.size(); i++) {
            ...
            if (percent != prevPercent) {
                log.info("Extracting from " + (i+1) + "/" + documents.size() + " (" + percent + "%) documents...\r");
                prevPercent = percent;
            }
